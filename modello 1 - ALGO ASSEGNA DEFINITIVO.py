...
filescrittura= open('risultati_10n6m1a_ALGORITMO ASSEGNAZIONE.txt', 'w')
print('Dati iniziali da file 10n6m1a.txt                  Modello 1\n\nLegenda:\ncolonna 1 istanza;\ncolonna 2 valore funzione obbiettivo;\ncolonna 3 stato della soluzione;\ncolonna 4 tempo di calcolo;\ncolonna 5 miglior LB.\n\n', file=filescrittura)
...
for i in range(5):
    ...
    for i in range(3):
        ...
        y =[[m.add_var(name='y({})({})'.format(i, t), var_type=BINARY) for t in range(T-s_tj[i])] for i in range(J+JD)]  
        ...
        for t in range(T-1):   
            m+= xsum(y[i][s] for i in range(J) for s in range(max(0, (t-s_tj[i]+1)), min(t+1, (T-s_tj[i])))) <= macchine, '(1c)'
          
        for i in range(JD):
           m+= xsum(y[i+n][t] for t in range(T-s_sj[i]) if t < T-s_tj[i]) == 1, '(1d)'

        for t in range(T-1): 
                   m+= xsum(y[i+n][s] for i in range(JD) for s in range(max(0, (t-s_sj[i]+1)), min(t+1,(T-s_tj[i])))) <= 1, '(1e)'

        for i in range(J):
            m+= Cmax >= xsum((t + s_tj[i])*y[i][t] for t in range(T-s_tj[i])), '(1f)'

        for i in range(J):
           for t in range(T-s_tj[i]):
               m+= y[i][t] == y[i+n][t], '(1g)'
